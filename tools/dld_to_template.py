...
GENERATED_MARKER = "<!-- generated by tools/dld_to_template.py; sha256="


def stamp_generated(body: str) -> str:
    """Sign generated content, so a later run can tell its own output from an edit.

    The digest covers the body exactly as written to disk — stripped of trailing
    whitespace — so that reading it back and re-hashing gives the same answer.
    Hashing anything else makes the check fire on every run, which trains people
    to ignore it.
    """
...
def is_untouched_generation(text: str) -> bool:
    """Whether this file is exactly what a previous run wrote, byte for byte."""
    marker = text.rfind(GENERATED_MARKER)
    if marker < 0:
        return False
    canonical = text[:marker].rstrip()
    stamped = text[marker + len(GENERATED_MARKER) :].strip().removesuffix("-->").strip()
    return hashlib.sha256(canonical.encode("utf-8")).hexdigest() == stamped


def preserve_hand_edits(report_path: Path, ip_name: str) -> Path | None:
    """Never destroy someone's writing to make room for generated text.

    The gaps report is regenerated on every parse, and reviewers were told — by
    this tool's own instructions — to record their decisions in it. Those
    decisions were then silently overwritten by the next extraction and, because
    `reports/` is gitignored, were gone for good. Decisions now belong in
    `decisions/<ip>.md`; this is the safety net for anything still written in the
    old place, and it fails loudly rather than quietly.
    """
    if not report_path.is_file():
        return None
    existing = report_path.read_text(encoding="utf-8")
    if is_untouched_generation(existing):
        return None
    kept = report_path.with_suffix(".hand-edited.md")
    kept.write_text(existing, encoding="utf-8")
    print(f"  NOTE: {report_path.name} had hand-written content; kept a copy at {kept.name}")
    print(f"        Move anything worth keeping into decisions/{ip_name}.md — that file is tracked")
    print("        and never regenerated. See decisions/README.md.")
    return kept
...
def render_gaps_report(ip_name: str, draft: dict[str, Any], gaps: list[str], open_items: list[str]) -> str:
    ...
    lines = [
        ...
        "",
        "> **This file is regenerated on every parse and is gitignored — nothing you",
        f"> write here survives.** Record decisions in `decisions/{ip_name}.md`, which is",
        "> tracked and never generated. See `decisions/README.md`.",
        "",
        "## Extracted",
        "",
...
def write_outputs(dld_path: Path, templates_dir: Path, reports_dir: Path) -> tuple[Path, Path, Path]:
    ...
    preserve_hand_edits(report_path, ip_name)
    report_path.write_text(stamp_generated(render_gaps_report(ip_name, draft, gaps, open_items)), encoding="utf-8")
