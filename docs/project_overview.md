...
| **SystemC model** (opt-in) | A second, independently-compiled delay model in C++/SystemC, only for an IP whose template names `systemc` in `ip.modeling_backends`. Every other IP is unaffected. | `systemc/models/<ip>.{h,cpp}` |
...
Beyond those, `run_ci.py` runs six further checks that are deliberately **not**
...
`review_model` — is fixed or dismissed by name in `decisions/<ip>.md`),
...
backend still compiles and passes its SystemC testbench — a no-op with zero
IPs opted in). Eleven gates in total; `python tools/run_ci.py --list` prints
all of them with their commands.
...
`python tools/run_ci.py` runs all eleven repo-wide gates in about a minute, and
the pre-push hook runs it before anything leaves the machine. The block below
is generated by `python tools/report_review_status.py --write` — never edit it
by hand, and `python tools/report_review_status.py --check` (an EXTRA_CHECKS
gate in `run_ci.py`) fails the moment it drifts from `reviews/*.findings.yaml`
and `decisions/*.md`. It deliberately doesn't claim a round count or a
lifetime finding total: a fixed finding leaves no durable trace once the code
changes (that's what makes its review stale), so only what the current repo
state can actually support is stated here.
...
| IP | What it models | Backends |
| --- | --- | --- |
| `arbitration_ip` | Hierarchical port/tenant/SQ arbitration with pending bitmaps, RR/WRR policy, burst-limited issue pipeline. | SimPy |
| `completion_ip` | Command completion scheduling with per-tenant QoS tokens, window-based refill, output backpressure. | SimPy |
